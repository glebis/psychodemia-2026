...
3. **coaching** - Coaching or mentoring session
   ...
   - Reflective questions from coach
   - Action planning, progress review

4. **therapy** - Psychotherapy or psychiatry session
   - Mental health discussion, anxiety, depression, ADHD
   - Therapeutic techniques (CBT, metacognitive therapy)
   - Medication discussion (antidepressants, dosage, side effects)
   - Emotional processing, coping strategies
   - Behavioral experiments, homework assignments

5. **mentoring** - Mentoring or lab 1-on-1 session
   - Skill transfer, teaching tools/workflows
   - Reviewing progress on previous tasks
   - Assigning new action items
   - Participant development tracking

6. **internal** - Internal team meeting
   - Project updates
   - Team coordination
   - Internal planning
   - Status reviews

Respond with ONLY the type identifier (leadgen/partnership/coaching/therapy/internal) and confidence (high/medium/low) in format:
TYPE: <type>
...
"""
    ...
    if meeting_type not in ['leadgen', 'partnership', 'coaching', 'therapy', 'mentoring', 'internal']:
