...
def generate_questions_therapy(extracted_data):
    """
    Generate clarifying questions for therapy session.
    ...
    if not followup.get('scheduled'):
        questions.append({
            ...
            "options": [
                {"label": "Yes, scheduled", "description": "Next session has confirmed date/time"},
                {"label": "No, not scheduled", "description": "No specific follow-up arranged"},
                {"label": "Regular cadence", "description": "Follows existing recurring schedule (e.g. weekly Fridays)"}
            ]
        })

    # Ask about session depth
    questions.append({
        "question": "How would you rate the therapeutic depth of this session?",
        "header": "Depth",
        "multiSelect": False,
        "options": [
            {"label": "Breakthrough", "description": "Major insight or emotional shift, new understanding"},
            {"label": "Deep", "description": "Meaningful exploration, good therapeutic work"},
            {"label": "Medium", "description": "Useful session, steady progress"},
            {"label": "Surface", "description": "Mostly check-in, light processing"}
        ]
    })

    # Ask about homework commitment
    if extracted_data.get('homework'):
        questions.append({
            "question": "How committed do you feel to the homework tasks?",
            "header": "Homework commitment",
            "multiSelect": False,
            "options": [
                {"label": "Very committed", "description": "Will definitely do all tasks"},
                {"label": "Mostly committed", "description": "Will try most, some feel hard"},
                {"label": "Uncertain", "description": "Not sure I'll follow through"},
                {"label": "Need to simplify", "description": "Too many tasks, need to prioritize"}
            ]
        })
...
def apply_answers_therapy(extracted_data, answers):
    """
    Apply user answers to therapy extracted data.
    """
    # Follow-up
    if 'followup' in answers:
        if answers['followup'] == 'Yes, scheduled':
    ...
    # Homework commitment
    if 'homework commitment' in answers:
        extracted_data['homework_commitment'] = answers['homework commitment']

    return extracted_data


def generate_questions_mentoring(extracted_data):
    """
    Generate clarifying questions for mentoring/lab session.
    """
    questions = []
    ...
    # Ask about progress pace
    questions.append({
        "question": "How would you rate the participant's progress pace?",
        "header": "Progress pace",
        "multiSelect": False,
        "options": [
            {"label": "Accelerating", "description": "Clearly gaining momentum, doing more than expected"},
            {"label": "Steady", "description": "Good pace, completing tasks, learning well"},
            {"label": "Stalled", "description": "Stuck, not completing actions, needs a different approach"}
        ]
    })
...
def apply_answers_mentoring(extracted_data, answers):
    """
    Apply user answers to mentoring extracted data.
    """
    # Follow-up
    if 'followup' in answers:
        if answers['followup'] == 'Yes, scheduled':
    ...
    # Progress pace
    if 'progress pace' in answers:
        pace_map = {
            'Accelerating': 'accelerating',
            'Steady': 'steady',
            'Stalled': 'stalled'
        }
        pace = pace_map.get(answers['progress pace'], 'steady')
        extracted_data.setdefault('session_quality', {})['progress_pace'] = pace
